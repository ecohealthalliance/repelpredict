...
model_object <-  nowcast_boost_model(
  disease_status_model = NULL, # for now looking only at cases
...
augmented_data_from_model_predict <- predict(object = model_object$cases_model, new_data = augmented_data_from_model)
hist(augmented_data_from_model_predict$.pred)
actual <- model_object$cases_model$pre$mold$outcomes$cases
predicted <- augmented_data_from_model_predict$.pred
...
actual <- augmented_data$cases
predicted <- augmented_data_predict$.pred
...
augmented_data_expanded_predict <- predict(object = model_object$cases_model, new_data = augmented_data_expanded)
hist(augmented_data_expanded_predict$.pred)
actual <- augmented_data_expanded$cases
predicted <- augmented_data_expanded_predict$.pred
plot(log10(actual), predicted)
