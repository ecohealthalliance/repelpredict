...
<details>
<summary>cases residuals by select disease</summary>
```{r cs-var-pd-by-disease, fig.height=10, fig.width=15}
importance_cases_disease <- importance_cases %>% 
  filter(str_starts(Feature, "disease_")) %>% 
  slice(1:5) %>% 
  mutate(disease = str_remove(Feature, "disease_")) %>% 
  pull(disease)

scored_data_cs %>% 
  filter(disease %in% importance_cases_disease) %>% 
    ggplot(., aes(x = cases_actual, y = cases_predicted, color = log10(cases_error))) +
  geom_point(alpha = 0.5) +
