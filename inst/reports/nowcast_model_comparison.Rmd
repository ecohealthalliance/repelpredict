...
### Disease Status
```{r ds-cm}
scored_data_ds_baseline <- scored_data_baseline$scored_tibble %>% 
...
scored_data_ds_boost <- scored_data_boost$scored_tibble %>% 
  select(taxa, country_iso3c, disease_status_actual, disease_status_predicted, disease_status_dirchange_actual, disease_status_dirchange_predicted) %>% 
...
importance_disease_status <- xgboost::xgb.importance(feature_names = colnames(boost_train_disease_status$data), model = boost_mod_disease_status)
head(importance_disease_status, n = 12)

top_vars <- importance_disease_status$Feature[1:12]
...
pd_disease_status <- map(top_vars, function(var){
  var_resp <- model_profile(explainer_disease_status, variable =  var , type = "partial")
  plot(var_resp, geom = "profiles") +
    labs(title = "", subtitle = "")
})
...
importance_cases <- xgboost::xgb.importance(feature_names = colnames(boost_train_cases$data), model = boost_mod_cases)
head(importance_cases, n = 12)

top_vars <- importance_cases$Feature[1:12]
...
pd_cases <- map(top_vars, function(var){
  var_resp <- model_profile(explainer_cases, variable =  var , type = "partial")
  plot(var_resp, geom = "profiles") + # ceteris-paribus (CP) profiles 
    labs(title = "", subtitle = "")
})
...
<details>
<summary>cases partial dependency by select disease (xgboost only) </summary>
...
cases_lag_pd_by_disease <- map(oie_high_importance_diseases, function(disease){
  
  col <- paste("disease", disease, sep = "_")
  which_disease <- which(boost_train_cases$data[,col] == 1)
  
  if(length(which_disease)){
    explainer_cases <- DALEX::explain(
      model = boost_mod_cases,
      data = boost_train_cases$data[which_disease,],
      y = boost_train_cases$label[which_disease],
      predict_function = predict,
      label = disease,
      verbose = FALSE
    )
    var_resp <- model_profile(explainer_cases, variable =  "cases_lag1" , type = "partial")
    plot(var_resp, geom = "profiles") + # ceteris-paribus (CP) profiles 
      labs(title = disease, subtitle = "")
  }
})

plot_grid(plotlist = cases_lag_pd_by_disease)
