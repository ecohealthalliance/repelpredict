...
mod <- read_rds(here::here("tmp/brms_mod_simp.rds"))
...
draws_sum <- draws %>% 
  ...
  summarize(r_disease_median = median(r_disease), 
            hd_low = bayestestR::hdi(r_disease)$CI_low,
            hd_high = bayestestR::hdi(r_disease)$CI_high) %>% 
  ungroup() %>% 
    mutate(name = str_replace(name, "_from_outbreaks", " from countries with existing outbreak"),
         name = str_replace(name, "ots_trade_dollars", "trade value of animal products"),
         name = str_replace(name, "fao_trade_", ""),
...
brms_coefs <- purrr::map(unique(draws_sum$name), function(var){
  dat <- draws_sum %>%
    filter(name == var) 
  # xmin <- min(dat$value)
  # xmax <- max(dat$value)
  dat <- dat %>% 
    filter(disease %in% get_oie_high_importance_diseases()) %>% 
    mutate(disease = str_replace_all(disease, "_", " ")) %>% 
    mutate(disease = forcats::fct_reorder(disease, r_disease_median))
  title <- var
  ggplot(dat, aes(x = disease, y = r_disease_median)) +
    geom_hline(aes(yintercept = 0), color = "gray50") +
    geom_point(size = 2) +
    geom_errorbar(aes(ymin = hd_low, ymax =hd_high),  width = 0.2) +
    labs(y = "Coefficient", x = "", title = title) +
    coord_flip() +
    theme_minimal() +
    theme(legend.position = "none", 
...
names(brms_coefs) <- unique(draws_sum$name)

```

```{r, include = FALSE}
src <- map_chr(seq_along(brms_coefs), ~ {
  tab_name <- names(brms_coefs[.x])
  knit_expand(text = c("<details>",
                       "<summary>{{tab_name}}</summary>",
                       "```{r tab-cat-2-{{.x}}, echo = FALSE, align = 'left'}", 
                       "brms_coefs[[{{.x}}]]",
                       "```",
                       "</details>"))
})
