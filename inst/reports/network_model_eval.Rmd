...
augment_disaggregated <- DBI::dbReadTable(conn, "network_lme_augment_disaggregated") %>% 
  as_tibble() %>% 
  filter(outbreak_start == "t") %>% 
  filter(disease == disease_select)  %>% 
  filter(month >= "2018-01-01") %>% 
  filter(country_origin != "NA") %>% 
  filter(country_iso3c == "DEU", month == "2018-01-01")

augment_disaggregated %>% 
  select(country_origin, fao_livestock_heads_from_outbreaks) %>% 
  mutate(fao_livestock_heads_from_outbreaks = as.numeric(fao_livestock_heads_from_outbreaks)) %>% 
  arrange(-fao_livestock_heads_from_outbreaks)
