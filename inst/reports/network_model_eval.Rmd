...
ggplot(lme_binoms, aes(y = outbreak_start_predict_prob_grp_median, x  = outbreak_start_actual_prob)) +
  ...
  labs(y = "Forecasted probability", x = "Actual probabilty",
       caption = paste("Forecasted probabilities are binned into", n_distinct(lme_binoms$outbreak_start_predict_prob_grp_median), "groups by distribution density.\nActual probabilities are binomial probabilities and confidence intervals for actual outbreaks occurring within the forecasted interval")) +
  theme_minimal() +
  theme(legend.position = "none",
...
residuals <- tibble::enframe(resid(lme_mod_baseline))

plot(lme_mod_baseline, disease ~ resid(.), abline = 0 )
...
lm_mod_sum <- summary(lme_mod_baseline)
lm_mod_sum
...
lme_predict <- augmented_data_val_bootstrap %>%
  select(-month, -outbreak_start) %>%
  mutate(outbreak_start_predict_prob = repel_predict(model_object_baseline, .)) %>% 
  distinct() %>% # getting unique conditions  (unique combination of values)
  drop_na(outbreak_start_predict_prob) %>% 
  mutate(outbreak_start_predict_prob_grp = cut_number(outbreak_start_predict_prob, n = 30)) %>%
  group_by(outbreak_start_predict_prob_grp) %>%
