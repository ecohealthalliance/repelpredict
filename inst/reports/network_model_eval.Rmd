...
#   arrange(-outbreak_start_predicted)
# 
# augment_disaggregated <- DBI::dbReadTable(conn, "network_lme_augment_disaggregated") %>% 
#   as_tibble() %>% 
#   mutate(month = as.Date(month)) %>% 
#   mutate(shared_borders_from_outbreaks = shared_borders_from_outbreaks == "t") %>% #temp
#   mutate_at(.vars = c("shared_borders_from_outbreaks", "ots_trade_dollars_from_outbreaks", "fao_livestock_heads_from_outbreaks"), as.numeric) %>% 
#   pivot_longer(cols = c("shared_borders_from_outbreaks", "ots_trade_dollars_from_outbreaks", "fao_livestock_heads_from_outbreaks"), names_to = "variable", values_to = "x_disagg") %>% 
#   filter(country_origin != "NA") %>% 
#   select(-outbreak_start) 
# 
# 
# hpai_var_importance2 <- hpai_var_importance %>% 
#   left_join(augment_disaggregated) %>% 
#   filter(variable != "shared_borders_from_outbreaks") %>% 
#   mutate(x_disagg = as.numeric(x_disagg)) %>% 
#   filter(x_disagg != 0)
# 
# hpai_var_importance2 %>% 
#   filter(country_iso3c == "DEU") %>% 
#   arrange(-x_disagg) %>% 
#   ggplot(aes(x = country_origin, y = x_disagg, fill = variable)) + 
#   geom_col() +
#   scale_y_log10()


# figure - for country/disease/month
# b(x1 + x2)
# beta * value for each contributor, colored by variable (bar chart?)
