...
status_switched <- scored_boost$scored_tibble %>% 
  mutate(ss = disease_status_dirchange_actual == "outbreak starts" &
           disease_status_dirchange_predicted == "outbreak starts" ) 
which_status_switched <- which(status_switched$ss)

status_switched_augmented <- forecasted_boost$augmented_data %>% 
  slice(which_status_switched) %>% 
  select(-disease_status)

rec <- boost_mod_disease_status %>% pull_workflow_prepped_recipe()

status_switched_mat <- bake(object = rec, new_data = status_switched_augmented) %>%
...
explainer_disease_status_sub <- DALEX::explain(
  model = boost_mod_disease_status_xg,
  data = status_switched_mat,
  y = rep(1, nrow(status_switched_mat)),
  predict_function = predict_raw,
  verbose = FALSE
)

vars <- c("log_veterinarians_per_taxa",  "log_taxa_population", "cases_lag_sum_border_countries", "cases_lag1", "cases_lag2", "cases_lag3")
vars <- c(vars, colnames(status_switched_mat)[str_starts(colnames(status_switched_mat), "control")])
